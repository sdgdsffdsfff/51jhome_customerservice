...
xxim.popchat = function(param){
    ...
    log.html = '<div class="layim_chatbox" id="layim_chatbox">'
            ...
            +'<div role="tabpanel" class="tab-pane" id="layim_chathistory"><iframe id="frame_history"></iframe></div>'
...
xxim.tabchat = function(param){
    ...
    $('#frame_history').attr("src", BASE_URL+"steadmin/serverol/chathistory?"+"serverid="+SERVERID+"&clientid="+param.id);
...
xxim.setSocket = function(socket){
...
xxim.transmit = function(){
    ...
    log.send = function(){
        ...
        if(data.content.replace(/\s/g, '') === ''){
            ...
            config.socket.emit('msg_from_server', {clientid: xxim.nowchat.id, msg: data.content});
            // setTimeout(function(){
            //     log.imarea.append(log.html({
...
xxim.appendMsg = function(data){
    var node = xxim.node;
    var keys = data.type + data.id;
    var log = {};
    //聊天模版
    log.html = function(param, type){
        return '<li class="'+ (type === 'me' ? 'layim_chateme' : '') +'">'
