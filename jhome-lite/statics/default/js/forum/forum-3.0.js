...
var jieLinforum = {
	...
	initList: function(){
		...
		$(document).on('click','.like',function(){
			...
			var obj = {'tid': $this.parent().attr('fid'), 'element': $this, 'confirm': 'like-on',};
	...
	initDetail: function(){
		...
		$('#like-b').on('click',function(){
			...
			status == 'clicked' ? url = delPraiseUrl : url = praiseUrl;
			var obj = {'tid': $this.parent().attr('fid'), 'element': $this};
	...
	//帖子列表页点赞 obj{'tid': 帖子id, element': click对象, 'confirm': 点赞class}, url: post url
	setLike: function(obj,url){
		...
		if(status != 'clicked'){	
			like_i.addClass('fadeIn');
			isNaN(num) || num <= 0 ? num = 1 : num += 1;
			like_num.html(num);
			$element.addClass(obj.confirm).data('status','clicked');
			$.post(url, {"fid": obj.tid});
		}else{
			like_i.removeClass('fadeIn');
			isNaN(num) || num <= 0 ? num = 0 : num -= 1;
			like_num.html(num);
			$element.removeClass(obj.confirm).data('status','');
			$.post(url, {"fid": obj.tid});
		}
	},
	//帖子详情页点赞 obj{'tid': 帖子id, element': click对象}, url: post url
