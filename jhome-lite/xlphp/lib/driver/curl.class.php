...
//设置http头,支持lighttpd服务器的访问
        $header = array();
        if (isset($this->setopt['header'])) {
            $header = $this->setopt['header'];
        }
        $header[] = 'Expect:';
        curl_setopt($this->ch, CURLOPT_HTTPHEADER, $header);
//设置 HTTP USERAGENT
        curl_setopt($this->ch, CURLOPT_USERAGENT, $this->setopt['userAgent'] ? $this->setopt['userAgent'] : (isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : ''));
//设置连接等待时间,0不等待
...
//获取的信息以文件流的形式返回，而不是直接输出。
     ...
     * @return 错误返回:false 正确返回:结果内容
     */
    public function put($url, $params = array(), $referer = '') {
        return $this->_request('PUT', $url, $params, array(), $referer);
    }
     ...
     * @return 错误返回:false 正确返回:结果内容
     */
    public function delete($url, $params = array(), $referer = '') {
        return $this->_request('DELETE', $url, $params, array(), $referer);
    }
