...
class Mydb {
    ...
    private function connect() {
        ...
        try {
            if ($this->config['dbtype'] == 'mysql') {
                $this->db = new PDO('mysql:host=' . $this->config['host'] . ';port=' . $this->config['port'] . ';dbname=' . $this->config['dbname'], $this->config['user'], $this->config['password'], array(PDO::ATTR_PERSISTENT => $this->config['pconnect']));
    ...
    function update($table, $arr, $where) {
        ...
        return $this->Exec($sql . ' ' . $this->formatCondition($where));
    }
