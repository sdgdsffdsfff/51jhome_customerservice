...
function F($name, $value = '', $path = '') {
    ...
    $path = $path ? $path : APP_PATH . 'runtime/data' . DIRECTORY_SEPARATOR;
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            // 缓存数据，目录不存在则创建
            setDir(dirname($filename), 0755);
            $_fcache[$saveName] = true;
            $value = json_encode($value);
            if (strExists($value, "'")) {
                $value = str_replace("'", '&#039;', $value);
            }
            return file_put_contents($filename, "<?php\n if (!defined('IN_XLP')){ exit('Access Denied');} return '" . $value . "';\n?>");
        }
    }
    if (!isset($_fcache[$saveName]) && is_file($filename)) {
        $_fcache[$saveName] = true;
    }
    // 获取缓存数据
    if (isset($_fcache[$saveName])) {
