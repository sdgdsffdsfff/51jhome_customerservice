...
$Document = array(
    'pageid' => 'goodscate-edit', //页面标示
    'pagename' => '编辑添加', //当前页面名称
...
<!-- content -->
  <section id="content">
    ...
    <div class="clearfix"> 
     <h4><i class="fa fa-edit"></i>分类编辑</h4> 
    </div> 
    <div class="row"> 
     <div class="col-sm-12"> 
      <section class="panel"> 
       <div class="panel-body"> 
        <form class="form-horizontal" id="cateAddForm" method="post" > 
         <div class="form-group"> 
          ...
          <div class="col-sm-3"> 
            <input type="text" name="name" id="name" placeholder="分类名称" class="form-control" value="<?php echo $rs['name'];?>" data-required="true"  />
          </div> 
         </div> 
         ...
         <div class="form-group"> 
          <label class="col-sm-1 control-label">是否显示</label> 
            <div class="row">
              <div class="col-xs-1 radio m-l-large"><label class="radio-custom"> <input type="radio" name="is_show" <?php if($rs['is_show']){echo 'checked="checked"';}?> value="1" /> <i class="fa fa-circle-o"></i>显示</label></div>
              <div class="col-xs-1 radio m-l-large"><label class="radio-custom"> <input type="radio" name="is_show" <?php if(!$rs['is_show']){echo 'checked="checked"';}?> value="0" /> <i class="fa fa-circle-o"></i>隐藏</label></div>
            </div>
         </div> 
         ...
         <div class="form-group"> 
          ...
          <div class="col-sm-1"> 
            <input type="number" name="sort"  id="sort" placeholder="" class="form-control" value="<?php echo $rs['sort'];?>" />
          </div> 
         </div> 
...
$(function(){
	...
	function getCateData(id,cateid){
		Msg.loading();
		$.getJSON('<?php echo U('goodscate/cate');?>',{"cid":id,"cateid":cateid},function(res){
			Msg.hide();
			if (res.status==1){
				$("#pid").html(res.data);
				selectCate();
  ...
  $('#cateAddForm').submit(function(){
    ...
    Msg.loading();
    $.post('<?php echo U('goodscate/save');?>',$(this).serialize(),function(result){
      Msg.hide();
      resetSubmit("#sub-ok",'提交');
      if (result.status==1){
	...
	getCateData(<?php echo $parentId;?>,<?php echo $rs['id'];?>);
})
</script>
<?php
