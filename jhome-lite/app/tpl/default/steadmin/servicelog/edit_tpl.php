        ...
        <div class="panel panel-servicelog">
          <div class="panel-body">
            <form class="form-horizontal" action="<?php echo U('servicelog/save');?>" method="post" id="form-fb1">
              <input type="hidden" name="fid" value="<?php echo $servicelog['fid'];?>">
              <div class="form-group">
                <label for="" class="col-sm-1 control-label">时间</label>
                <div class="col-lg-2">
                  <input class="form-control fbtime" type="text" value="<?php echo $servicelog['fb_time']; ?>" name="fbtime" />
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-lg-2">
                  <select name="fbtype" id="fb-typelist"  class="form-control">
                    ...
                    <?php foreach ($types as $key => $value) { ?>
                      <option value="<?php echo $key;?>"<?php if($servicelog['type_id'] == $key) echo ' selected="selected"';?>><?php echo $value['name'];?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="col-lg-2">
                  <input type="text" class="form-control" name="order_id" value="<?php if($servicelog['order_id']){  echo $servicelog['order_id'];}?>" id="input_order_id">
                </div>
              </div>
              <div class="form-group">
                <label for="input_client_username" class="col-sm-1 control-label">用户名</label>
                <div class="col-lg-2">
                  <input type="text" class="form-control" name="username" value="<?php if($servicelog['client_username']){  echo $servicelog['client_username'];}?>" id="input_username">
                </div>
              </div>
              <div class="form-group">
                <label for="input_phone" class="col-sm-1 control-label">手机号</label>
                <div class="col-lg-2">
                  <input type="text" class="form-control" name="phone" value="<?php if($servicelog['phone']){  echo $servicelog['phone'];}?>" id="input_phone">
                </div>
              </div>                
              <div class="form-group">
                <label for="" class="col-sm-1 control-label">反馈内容</label>
                <div class="col-lg-5">
                  <textarea class="form-control servicelogContent" name="fbcontent" cols="30" rows="10" ><?php echo $servicelog['servicelog']; ?></textarea>
                </div>
              </div>
              <div class="form-group">
                <label for="" class="col-sm-1 control-label">附件</label>
                <div class="col-sm-3">
                  <input type="text" name="fbupload" id="fb_upload" placeholder="" class="form-control" value="<?php echo $servicelog['upload'];?>" />
...
$(function(){
  ...
  $('#btn-savefb').click(function(event){
    ...
    if(!clicked){
     ...
     Msg.loading();
      $.ajax({
        url: "<?php echo U('servicelog/save');?>",
        type: 'POST',
